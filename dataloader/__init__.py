...
def getDataLoader(datapath, dataset='sceneflow', trainCrop=(256, 512), batchSizes=(12, 11),
    ...
    trainImgLoader = torch.utils.data.DataLoader(
        fileLoader.myImageFloder(*pathsTrain, trainCrop=trainCrop,
                                 ...
                                 mode=mode, mask=mask),
        batch_size=batchSizes[0], shuffle=True, num_workers=1, drop_last=False) if batchSizes[0] > 0 else None
    ...
    testImgLoader = torch.utils.data.DataLoader(
        fileLoader.myImageFloder(*pathsTest, trainCrop=trainCrop,
                                 ...
                                 mask=mask),
        batch_size=batchSizes[1], shuffle=False, num_workers=1, drop_last=False) if batchSizes[1] > 0 else None
